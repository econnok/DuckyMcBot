    ...
    s.getPlayerSummaries({
      ...
      callback: function(err,data){
        ...
        mika.getMatch(matchid[4]).then((match) => {
          ...
          var muhchat = match.chat
         // console.log(match.chat)
         var wah = [];
          muhchat.forEach( function (arrayItem)
            {
              if (arrayItem.type == 'chat')
              wah.push(arrayItem);
             });
           ...
           // var chatobj = JSON.parse(randomProperty(match.chat));
          var coolchat = randomProperty(wah)
          if (coolchat.hasOwnProperty('key')) {
            // your code here
            msg.channel.sendMessage('Player ' + coolchat.unit + ' had this to say: "' + coolchat.key+'"');
            msg.channel.sendMessage(coolchat.key,{ tts:true})
          }
